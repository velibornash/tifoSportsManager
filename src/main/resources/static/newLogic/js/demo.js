...
function handleSeasonFlowResponse(data, button, defaultLabel) {
    ...
    if (action === 'START_MATCH') {
        ...
        window.location.href = `/newLogic/realisticDemo.html?matchId=${matchId}&mode=live`;
        return;
    }
    ...
    if (action === 'WEEK_PREPARED') {
        ...
        if (target === 'match') {
            if (data.userMatchId) {
                window.location.href = `/newLogic/realisticDemo.html?matchId=${data.userMatchId}&mode=live`;
            } else {
        ...
        if (target === 'results') {
            window.location.href = `/newLogic/simulateAllResults.html`;
            return;
        }
        return;
    }
