...
export function createTacticEditorView(deps) {
    ...
    async function loadTacticEditor() {
        ...
        const saveDraft = () => {
            try {
                localStorage.setItem(DRAFT_KEY, JSON.stringify({
                    ...
                    draftVersion: state.draftVersion, activePossession: state.activePossession,
        ...
        const getSlotTarget = (slotKey, ballStateKey, possessionContext) => getRule(slotKey, ballStateKey, possessionContext) || slotByKey.get(slotKey)?.anchorCellKey || 'CELL_2_2';
        ...
        const clearFocusedRule = () => { if (state.focusedSlot && state.activeBallState) setRule(state.focusedSlot, state.activeBallState, state.activePossession, ''); };
        ...
        const buildPlayerMarkersHtml = () => {
            const byCell = {};
            slots.forEach(slot => { const cellKey = getSlotTarget(slot.slotKey, state.activeBallState, state.activePossession); if (!byCell[cellKey]) byCell[cellKey] = []; byCell[cellKey].push(slot); });
        ...
        const bindEvents = () => {
            ...
            mainContent.querySelectorAll('[data-te-possession]').forEach(btn => { btn.addEventListener('click', () => { state.activePossession = btn.getAttribute('data-te-possession') || state.activePossession; saveDraft(); render(); }); });
            ...
            mainContent.querySelectorAll('[data-te-drop]').forEach(zone => {
                ...
                zone.addEventListener('drop', event => {
                    ...
                    if (payload.type === 'slot' && payload.slotKey && tck) { state.focusedSlot = payload.slotKey; setRule(payload.slotKey, state.activeBallState, state.activePossession, tck); render(); }
                });
            });
        ...
        const render = () => {
            ...
            const focusedTarget = state.focusedSlot ? getSlotTarget(state.focusedSlot, state.activeBallState, state.activePossession) : '';
            ...
            const possessionButtons = POSSESSION_OPTS.map(ctx => `<button type="button" class="te-toggle ${ctx === state.activePossession ? 'active' : ''}" data-te-possession="${ctx}">${ctx === 'WE_HAVE_BALL' ? 'We have ball' : 'Opponent has ball'}</button>`).join('');
