...
/**
 * Self-contained formation slot catalog — defines anchor positions for supported
 * formations. The editor uses a grid with ROWS/COLS INDEXED FROM 0, so anchor
 * keys are "CELL_r_c" (0-based). Conversion to 1-based field positions happens in
 * TacticsRules.parseCell().
 */
public class FormationSlotCatalog {
    ...
    public List<String> getSupportedBallStates() { return SUPPORTED_BALL_STATES; }
    public List<String> getSupportedTargetCells() { return SUPPORTED_TARGET_CELLS; }
}