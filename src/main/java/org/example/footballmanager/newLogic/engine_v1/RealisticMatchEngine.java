...
import org.example.footballmanager.newLogic.dto.TacticsSetPieceDTO;
...
import org.example.footballmanager.newLogic.model.event.SetPieceEvent;
...
public class RealisticMatchEngine {
    ...
    private void handleShot(MatchRuntime rt, Match match, int minute,
        ...
        if (shotBlocker != null && shouldTriggerShotDeflection(rt, shooter, shotBlocker, openGoal)) {
            ...
            eventGenerator.createShotMissedEvent(rt, match, minute, shooter, duelResult.xG());
        ...
        if (duelResult.goal()) {
            ...
            GoalEvent goalEvent = eventGenerator.createGoalEvent(rt, match, minute, shooter, assistant, duelResult.xG());
            log.info("⚽ GOAL! {} scores for {} (xG: {:.2f})", shooter.getName(), ballTeam, duelResult.xG());
        ...
        } else if (!openGoal && goalkeeper != null && duelResult.saved()) {
            movePlayerTowardsGoal(rt, shooter, ballTeam, 6.0);
            eventGenerator.createShotSavedEvent(rt, match, minute, shooter, goalkeeper, duelResult.xG());
            log.info("🧤 SAVE! {} saved by {} (xG: {:.2f})", shooter.getName(), goalkeeper.getName(), duelResult.xG());

            PlayerPositionDTO goalkeeperPos = getPlayerPosition(rt, goalkeeper);
            double saveOutcomeRoll = random.nextDouble();
            double reboundChance = clamp(0.28 + duelResult.xG() * 0.34, 0.28, 0.54);
    ...
    private void maybeIssueRecklessDuelCard(MatchRuntime rt,
        ...
        if (result != null && result.attackerWins()) {
            chance += 0.006;
        }
        if (defender.getPosition() == Position.DEF) {
            chance += 0.004;
    ...
    private void handleFoul(MatchRuntime rt, Match match, int minute, Player attacker, Player defender) {
        ...
        if (isInPenaltyBox(attackerPos, attacksRight)) {
            ...
            DuelResolver.DuelResult penResult = DuelResolver.resolvePenalty(toSnapshot(rt, effectiveTaker), toSnapshot(rt, goalkeeper));
            ...
            if (penResult.goal()) {
                ...
                goalEvent = eventGenerator.createGoalEvent(rt, match, minute, effectiveTaker, null, penResult.xG());
                log.info("⚽ PENALTY GOAL! {} scores for {} (xG: {:.2f})", effectiveTaker.getName(), attackingTeam, penResult.xG());
                ...
                rt.pendingReceiverId = kickoffPlayer != null ? Math.toIntExact(kickoffPlayer.getId()) : null;
                rt.lastTouchTeam = restartTeam;
            } else if (goalkeeper != null && penResult.saved()) {
                eventGenerator.createShotSavedEvent(rt, match, minute, effectiveTaker, goalkeeper, penResult.xG());
                ...
                eventGenerator.createShotMissedEvent(rt, match, minute, effectiveTaker, penResult.xG());
                log.info("❌ PENALTY MISSED! {} (xG: {:.2f})", effectiveTaker.getName(), penResult.xG());
        ...
        boolean direct = estimateDistanceToGoal(rt, attacker, attackingTeam) <= 24.0;
        String freeKickSide = "HOME".equals(attackingTeam) ? "HOME" : "AWAY";
        SetPieceEvent fk = new SetPieceEvent(minute, rt.tick, freeKickSide,
                effectiveTaker.getId(), effectiveTaker.getName(),
                SetPieceEvent.SetPieceType.FREE_KICK, 50.0, 50.0);
        rt.runtimeEvents.add(fk);
        rt.lastTouchTeam = attackingTeam;
        executeFreeKickRestart(rt, match, minute, effectiveTaker, attackerPos, attackingTeam, direct, upperSide);
    ...
    private PlayerSnapshot toSnapshot(MatchRuntime rt, Player player) {
        PlayerPositionDTO pos = getPlayerPosition(rt, player);
        String team = getTeam(player, rt);
        return PlayerSnapshot.fromPlayer(player, team,
                pos != null ? pos.getX() : 50.0,
                pos != null ? pos.getY() : 50.0);
    }
    ...
    private void maybeCreateVarReview(GoalEvent goal, PenaltyEvent penalty, MatchRuntime rt, Match match, int minute) {
        ...
        log.info("VAR Review at minute {}", minute);

        if (goal != null) {
            if (random.nextDouble() < 0.80) {
                log.info("VAR Decision: Goal Confirmed");
            } else {
                log.info("VAR Decision: Goal Overturned ({})", randomOverturnReason());
            ...
            if (random.nextDouble() < 0.80) {
                log.info("VAR Decision: Penalty Confirmed");
            } else {
                log.info("VAR Decision: Penalty Overturned (encroachment)");
            }
        }
    }
