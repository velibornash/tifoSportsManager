...
import org.example.footballmanager.newLogic.engine_v1.PhysicsEngine;
...
public final class SetPieceHandler {
    ...
    public static RestartParams handlePenalty(MatchState state, int minute, long fouledPlayerId) {
        ...
        return new RestartParams("PENALTY", spotX, 50.0,
            taker != null ? taker.id() : -1, attackingTeam,
            MatchState.StoppageType.PENALTY, 50, "PENALTY");
    }
    ...
    public static void resolveCornerDelivery(MatchState state, int minute) {
        ...
        if (attacker != null && defender != null) {
            var result = DuelResolver.resolveTackle(
                PlayerSnapshot.fromPlayer(attacker, attackingTeam, targetX, targetY),
                PlayerSnapshot.fromPlayer(defender, defendingTeam, targetX, targetY));
    ...
    private static Player findSetPieceTaker(MatchState state, String team) {
        List<Player> players = "HOME".equals(team) ? state.homePlayers() : state.awayPlayers();
        // For corners and free kicks, prefer player with best passing + technique
        return players.stream()
            .filter(p -> p.position() != Position.GK)
            .max(java.util.Comparator.comparingDouble(p -> 
                p.skills().passing() * 0.6 + p.skills().technique() * 0.4))
    ...
    private static Player findPenaltyTaker(MatchState state, String team) {
        List<Player> players = "HOME".equals(team) ? state.homePlayers() : state.awayPlayers();
        // Prefer attacker with best shooting + technique
        return players.stream()
            .filter(p -> p.position() == Position.ATT || p.position() == Position.WNG)
            .max(java.util.Comparator.comparingDouble(p -> 
                p.skills().shooting() * 0.7 + p.skills().technique() * 0.3))
            .orElseGet(() -> players.stream()
                .filter(p -> p.position() != Position.GK)
                .max(java.util.Comparator.comparingDouble(p -> p.skills().shooting()))
                .orElseGet(() -> players.stream().filter(p -> p.position() != Position.GK).findFirst().orElse(null)));
    }
    ...
    private static void placePenaltySupportPlayers(MatchState state, String attackingTeam, Player taker, Player goalkeeper) {
        ...
        for (String team : List.of(attackingTeam, state.oppositeTeam(attackingTeam))) {
            ...
            int idx = 0;
            for (var p : players) {
                if (p.id() == takerId || p.id() == gkId) continue;
