...
import org.example.footballmanager.newLogic.dto.GameStateDTO;
...
public class MatchPlaybackEngine {
    ...
    private Comparator<MatchEvent> buildEventComparator(int fallbackTicksPerMinute) {
        return Comparator.<MatchEvent>comparingInt(
                ...
                .thenComparingInt(MatchEvent::minute)
                .thenComparing(e -> 0L);
    }
    ...
    private int resolveEventTick(MatchEvent event, int fallbackTicksPerMinute) {
        ...
        return Math.max(0, event.minute() * safeTicksPerMinute);
    }
    ...
    private Map<String, String> resolveTeamNames(Match match, List<MatchEvent> events) {
        ...
        if ((homeName == null || awayName == null) && events != null) {
            for (MatchEvent event : events) {
                if (event instanceof org.example.footballmanager.newLogic.model.event.MatchStartEvent startEvent) {
                    if (homeName == null) {
                        homeName = startEvent.homeTeamName();
                    }
                    if (awayName == null) {
                        awayName = startEvent.awayTeamName();
                    }
                }
            }
        }
