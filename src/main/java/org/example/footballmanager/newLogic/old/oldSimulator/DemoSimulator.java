package org.example.footballmanager.newLogic.old.oldSimulator;
...
import org.example.footballmanager.newLogic.old.oldService.DemoMatchRuntime;
import org.example.footballmanager.newLogic.util.match.MatchContext;
import org.example.footballmanager.newLogic.util.events.MatchEventFactory;
import org.example.footballmanager.newLogic.util.events.MatchEventMapper;
...
public class DemoSimulator {
    ...
    private void batchSaveMatchEvents(Match match, List<MatchEvent> events) {
        ...
        for (MatchEvent e : events) {
            if (e instanceof GoalEvent) continue;
            em.persist(e);
    ...
    private void batchSaveGoalEvents(Match match, List<GoalEvent> goals, List<Player> homePlayers, List<Player> awayPlayers) {
        ...
        for (GoalEvent g : goals) {
            if (g.scorerId() > 0) {
                Player scorer = homePlayers.stream().filter(p -> p.getId() == g.scorerId()).findFirst()
                        .orElseGet(() -> awayPlayers.stream().filter(p -> p.getId() == g.scorerId()).findFirst().orElse(null));
                if (scorer != null) {
            ...
            if (g.assistantId() != null && g.assistantId() > 0) {
                Player assistant = homePlayers.stream().filter(p -> p.getId() == g.assistantId()).findFirst()
                        .orElseGet(() -> awayPlayers.stream().filter(p -> p.getId() == g.assistantId()).findFirst().orElse(null));
                if (assistant != null) {
    ...
    public DemoMatchRuntime simulateMatch(Match match, Crowd crowd, Referee referee, Tactics homeTactics, Tactics awayTactics, List<Player> homePlayers, List<Player> awayPlayers, ScheduledExecutorService scheduler) {
        ...
        MatchStartEvent startEvent = new MatchStartEvent(1, 0, match.getHomeTeam().getName(), match.getAwayTeam().getName());
                ...
                try {
                    ...
                    if (currentMinute < 91) {
                        ...
                        if (event != null)
                        {
                            ...
                            log.info("[{}'] Event: {}", currentMinute, event.type());
                            ...
                            if (event instanceof GoalEvent goal) {
                                ...
                                if (goal.scorerId() > 0) {
                                    Player scorer = homePlayers.stream().filter(p -> p.getId() == goal.scorerId()).findFirst()
                                            .orElseGet(() -> awayPlayers.stream().filter(p -> p.getId() == goal.scorerId()).findFirst().orElse(null));
                                    if (scorer != null) {
                                        scorer.setTotalGoals(scorer.getTotalGoals() + 1);
                                    }
                                }
                            ...
                            if (event instanceof PenaltyEvent pen ) {
                                if (pen.scored()) {
                                    ...
                                    GoalEvent goal = new GoalEvent(currentMinute, 0, pen.takerId(), pen.takerName(), null, null, pen.teamSide(), pen.xG(), rt.homeGoals + ("HOME".equals(pen.teamSide()) ? 1 : 0), rt.awayGoals + ("AWAY".equals(pen.teamSide()) ? 1 : 0));

                                    if ("HOME".equals(pen.teamSide())) {
                                        rt.homeGoals++;
                                    } else {
                                        rt.awayGoals++;
                                    }
                                    if (pen.takerId() > 0) {
                                        Player scorer = homePlayers.stream().filter(p -> p.getId() == pen.takerId()).findFirst()
                                                .orElseGet(() -> awayPlayers.stream().filter(p -> p.getId() == pen.takerId()).findFirst().orElse(null));
                                        if (scorer != null) {
                                            scorer.setTotalGoals(scorer.getTotalGoals() + 1);
                                        }
                                    }
                                    ...
                                    log.info("[{}'] Event: {}", currentMinute, goal.type());
                    ...
                    {   MatchEndEvent endEvent = new MatchEndEvent(91, 0, rt.homeGoals, rt.awayGoals);
                        ...
                        log.info("[91'] Event: {}", endEvent.type());}
                    else
                    {
    ...
    public Match finalizeMatchResult(Match match,
        ...
        rt.homeGoals = (int) rt.runtimeGoals.stream()
                .filter(g -> "HOME".equals(g.teamSide()))
                .count();

        rt.awayGoals = (int) rt.runtimeGoals.stream()
                .filter(g -> "AWAY".equals(g.teamSide()))
                .count();
        ...
        homePlayers = assignRatings(homePlayers, rt.runtimeGoals);
        awayPlayers = assignRatings(awayPlayers, rt.runtimeGoals);

        List<CardEvent> cards = rt.runtimeEvents.stream()
                        .filter(e -> e instanceof CardEvent).map(e -> (CardEvent) e).toList();

        savePlayerStats(match, homePlayers, rt.runtimeGoals, cards);
        savePlayerStats(match, awayPlayers, rt.runtimeGoals, cards);
    ...
    private void simulateInjuriesAndCards(List<Player> players, Match match) {
        for (Player player : players) {
            if (random.nextDouble() < 0.05) {
                InjuryEvent injury = new InjuryEvent(random.nextInt(90) + 1, 0, player.getId(), player.getName(),
                        player.getTeam() != null ? "HOME" : "AWAY");
            }
            if (random.nextDouble() < 0.1) {
                CardEvent card = new CardEvent(random.nextInt(90) + 1, 0, player.getId(), player.getName(),
                        player.getTeam() != null ? "HOME" : "AWAY", CardEvent.CardType.YELLOW);
            }
        }
    }
    private void savePlayerStats(Match match, List<Player> players,
        ...
        for (Player player : players) {
            ...
            long yellowCards = allCards.stream()
                    .filter(c -> c.cardType() == CardEvent.CardType.YELLOW && c.playerId() == player.getId())
                    .count();

            long redCards = allCards.stream()
                    .filter(c -> c.cardType() == CardEvent.CardType.RED && c.playerId() == player.getId())
                    .count();
    ...
    private String generateMatchReport(Match match,
        ...
        rt.runtimeGoals.forEach(g -> {
            String scorerName = g.scorerName();
    ...
    private void performSubstitution(Match match, MatchContext context, List<Player> teamPlayers, boolean isHomeTeam) {
        ...
        SubstitutionEvent sub = new SubstitutionEvent(context.getCurrentMinute(), 0, out.getId(), out.getName(), in.getId(), in.getName(),
    ...
    private boolean isHomeTeam(MatchEvent event) {
        if (event instanceof GoalEvent goal) return "HOME".equals(goal.teamSide());
        if (event instanceof SubstitutionEvent sub) return "HOME".equals(sub.teamSide());
        return false;
    }
}