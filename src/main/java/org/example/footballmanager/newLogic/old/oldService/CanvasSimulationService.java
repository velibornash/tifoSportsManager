package org.example.footballmanager.newLogic.old.oldService;
...
public class CanvasSimulationService {
    ...
    private final MatchEventWSHandler webSocketHandler;
...
// Ako postoji stari scheduler i još nije ugašen → ugasi ga
        ...
        for (int i = 1; i <= 11; i++) {
            players.add(new PlayerPositionDTO(i, "HOME", 10 + random.nextDouble() * 35, 10 + random.nextDouble() * 80,0,0));
        }
        for (int i = 12; i <= 22; i++) {
            players.add(new PlayerPositionDTO(i, "AWAY", 65 + random.nextDouble() * 30, 10 + random.nextDouble() * 80,0,0));
        }
        ...
        scheduler.scheduleAtFixedRate(() -> {
            ...
            webSocketHandler.broadcast(0L, state);
            tick[0]++;
